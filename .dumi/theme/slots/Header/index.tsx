...
import { useLocation,useAppData } from 'dumi';
...
const Header: React.FC = () => {
  ...
  const onLangChange = useCallback(() => {
    ...
    if (utils.isLocalStorageNameSupported()) {
      console.log(utils.isZhCN(pathname))
      localStorage.setItem('locale', utils.isZhCN(pathname) ? 'en-US' : 'zh-CN');
    }
    
    let newpath = utils.getLocalizedPathname(pathname, !utils.isZhCN(pathname), search).pathname;
    if (newpath.startsWith("/")) { 
      newpath = newpath.slice(1)
    }
    if (!newpath.startsWith(basename)) { 
      newpath  = basename + newpath
    }
    const href:string = currentProtocol +
      currentHref.replace(
        window.location.pathname,
        newpath,
      );
    // href = href.inclide(basebame)
    // console.log(href.startsWith(basename));
